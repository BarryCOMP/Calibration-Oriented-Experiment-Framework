...
PROFILE_PREFILL_URLS="${PROFILE_PREFILL_URLS:-http://127.0.0.1:30000}"
PROFILE_ACTIVITIES="${PROFILE_ACTIVITIES:-GPU}"
PROFILE_START_STEP="${PROFILE_START_STEP:-}"
PROFILE_STEPS="${PROFILE_STEPS:-}"
BENCH_EXTRA_ARGS="${BENCH_EXTRA_ARGS:-}"

read -r -a PREFILL_URL_ARR <<< "${PROFILE_PREFILL_URLS}"
...
CMD=(
  ...
  --pd-separated
  --profile-prefill-url "${PREFILL_URL_ARR[@]}"
)
...
echo "Profiling PD prefill worker(s) with torch profiler..."
echo "Traffic endpoint: ${BASE_URL}"
echo "Profile target (prefill): ${PROFILE_PREFILL_URLS}"
